...
pub fn get_version() -> String {
    ...
    if let Ok(latest_tag) = get_latest_tag() {
        ...
        if is_head_newer_than_tag(&latest_tag) {
            // If current commit is newer, increment version number
...
/// Check if current HEAD is newer than the specified tag
...
fn increment_version(version: &str) -> Result<String, Box<dyn std::error::Error>> {
    ...
    // If pre-release identifier exists, increment pre-release version number
...
mod tests {
    ...
    fn test_version_format() {
        ...
        // If it's refs/tags/ format, it should contain version number
